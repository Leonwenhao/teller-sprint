...
**2. Latency tail — occasional queries exceed 3 minutes.** About 5 % of queries will see a first-attempt stall, trigger an automatic retry, and come back correct at 2–3× typical latency. You'll see exactly one stderr line: `teller: model timed out after 600s, retrying (attempt 2/2)...`. The answer is still correct when it returns. This is a known pattern of MiniMax M2.5 tail latency, not a Teller bug; we built the retry specifically for it. If the log line appears, nothing else about the answer changes.
